...
%\title{The Spectral Gap of Graphs Arising From Substring Reversals}
%\author{Fan Chung \and Josh Tobin}

%\maketitle

%\abstract{
%  The substring reversal graph $R_n$ is the graph whose vertices are the
%  permutations $S_n$, and where two permutations are adjacent if one is
%  obtained from a substring reversal of the other.  We determine the
%  spectral gap of $R_n$, and show that its spectrum contains many integer
%  values.  Further we consider a a family of graphs that generalize the
%  prefix reversal (or pancake flipping) graph, and show that every graph
%  in this family has adjacency spectral gap equal to one.
%}
...
such as genome arrangements and sequencing  (see \cite{BafnaPevzner1996}, \cite{Hannenhalli1996}, \cite{ks}). 
...
Chitturi et al. \cite{Chitturi2009}, improving on a previous bound of $\frac{5}{3} n$ given by
 Gates and Papadimitriou \cite{Gates1979} in 1979.  The best lower bound is
$f(n) \geq 15 \left\lfloor \frac{n}{14} \right\rceil$, which is due to 
Heydari and Sudborough \cite{HeydariSudborough}.  Recently it was shown that the problem of  determining the exact minimum number of flips
to transform  one permutation $\tau_1$ into another permutation $\tau_2$, for two
given permutations $\tau_1$ and $\tau_2$, is NP--hard \cite{Bulteau2015}.
  In \cite{Cesi2009}, it 
...
with particular interest in their spectral gap.  In \cite{Lubotzky}, 
...
construction of such a family was found in \cite{Kassabov2005}.  For many 
...
For undefined terminology, the reader is referred to  \cite{cgn}.
...
with transition probability matrix $P=D_G^{-1} A_G$ is exactly $
\lambda_1^{-1}$ (see \cite{cgn}).  We will denote the eigenvalues of 
...
In proving Theorem~\ref{thm:main} and Theorem~\ref{thm:rev_eig}, we will rely heavily on graph coverings, an idea developed in \cite{coverings}.  A short overview is presented here.  Let $G$ and $ \tilde{G}$ be two weighted graphs.  Then $\tilde{G}$ is a \textit{covering} of $G$ if there is a surjection $\pi : V(\tilde{G}) \to V(G)$ satisfying the following two properties:
\begin{itemize}
 ...
 \end{equation}
\end{itemize}
As $\pi$ is a surjection, it can alternatively be viewed as a partition of the vertices of $V(\tilde{G})$ into $|V(G)|$ sets.  With this interpretation, the above definition can be seen as a generalization of an \textit{equitable partition}; see, for example, \cite{agt}.
We say that $G$ is a {\it projection} of $\tilde{G}$ via the mapping $\pi$ if $\tilde{G}$ is a covering of $G$ under $\pi$.


The virtue of a graph covering is that there is a strong correspondence between the eigenvalues of a covering graph and the eigenvalues of the projection.  This correspondence is the content of the following theorem, which is proved in \cite{coverings}.
...
$\mathcal{P}_n + I$ can be derived by using the Plancherel formula (see \cite{cgn}):
\begin{align*}
...
%\bibitem{AMM} H. Dweighter, Amer. Math. Monthly 82 (1) 1010 (1975).
%\bibitem{BafnaPevzner1996} V. Bafna, P. Pevzner, Genome rearrangements and sorting by reversals, \emph{SIAM Journal on Computing}, \textbf{25} (2)  (1996) 272--289.
%\bibitem{Bulteau2015} L. Bulteau, G. Furtin, I. Rusu, Pancake Flipping is Hard, \emph{Journal of Computer and System Sciences}, \textbf{81} (8)  (2015) 1556--1574.
%\bibitem{Cesi2009} F. Cesi, Cayley graphs on the symmetric group generated by initial reversals have unit spectral gap, \emph{The Electronic Journal of Combinatorics}, \textbf{16}(1) (2009).
%\bibitem{Chitturi2009} B. Chitturi, W. Fahle, Z. Meng, L. Morales, C.O. Shields, I.H. Sudborough, W. Voit, An upper bound for sorting by prefix reversals, \emph{Theoretical Computer Science} 410 (36), 3372 (2009).
%\bibitem{coverings} F. Chung, S.-T. Yau, Coverings, heat kernels and spanning trees, \emph{Journal of Combinatorics}, \textbf{6}, (1998) 163--184 .
%\bibitem{cgn} F. Chung, \emph{Spectral Graph Theory} AMS Publications, Providence (1997).
%\bibitem{FlattoEtAl1985} L. Flatto, A. M. Odlyzko, D. B. Wales, Random shuffles and group representations, \emph{The Annals of Probability},  (1985) 154--178.

%\bibitem{Friedman2000} J. Friedman, On Cayley graphs on the symmetric group generated by tranpositions, \emph{Combinatorica}, \textbf{20}(4) (2000).
%\bibitem{Gates1979} W. Gates, C. Papadimitriou, Bounds For Sorting By Prefix Reversal, \emph{Discrete Mathematics}, \textbf{27}, (1979) 47--57.
%\bibitem{agt} C. Godsil, G. Royle, \emph{Algebraic Graph Theory}, Springer, New York (2001).
%\bibitem{GrossTucker} J. Gross, T. Tucker, \emph{Topological graph theory},
%Courier Corporation (1987).
%\bibitem{GunnellsEtAl2007} P. E. Gunnells, R. A. Scott, B. L. Walden, On certain integral Schreier graphs of the symmetric group, \emph{The Electronic Journal of Combinatorics}, \textbf{14}(1) (2007).
%\bibitem{Hannenhalli1996}, Hannenhalli, Sridhar. "Polynomial-time algorithm for computing translocation distance between genomes." Discrete applied mathematics 71.1-3: 137-151 (1996).
%\bibitem{HeydariSudborough} M. H. Heydari, I. H. Sudborough, On the diameter
%of the pancake network, \emph{J. Algorithms}, \textbf{25} (1) (1997)  67--94.
%\bibitem{HornJohnson} R. Horn, C. Johnson, \emph{Matrix Analysis}, Cambridge University Press, Cambridge (1985).
%\bibitem{Kassabov2005} M. Kassabov, Symmetric groups and expanders, \emph{Electronic Research Announcements of the American Mathematical Society}, \textbf{11}(6) (2005).
%\bibitem{ks} J. Kececioglu and D. Sankoff, Exact and approximation algorithms for sorting by reversals, with application to genome rearrangement, {\it Algorithmica}  \textbf{13} (1995), 180--210
%\bibitem{LiMeng2008} R. Li, J. Meng, Reversals Cayley graphs of symmetric groups, \emph{Information Processing Letters}, \textbf{109}(2) (2008).
%\bibitem{Lubotzky} A. Lubotzky, Cayley graphs: eigenvalues, expanders and random walks, \emph{London Mathematical Society Lecture Note Series}, (1995) 155--190.
